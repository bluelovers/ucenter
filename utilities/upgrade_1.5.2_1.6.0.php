...
/*
	...
	$Id: upgrade_1.5.2_1.6.0.php 1097 2011-05-17 02:13:13Z svn_project_zhangjie $
*/
...
if(file_exists($lock_file)) {
	showheader();
	showerror('升級被鎖定，說明您已經進行過升級操作，如需繼續升級請刪除 '.str_replace(UC_ROOT, '', $lock_file).' 文件，再嘗試本操作');
	showfooter();
}
...
?>
	...
	<p>本程序用於升級 UCenter 1.5.2 到 UCenter 1.6.0</p>
	<p>運行本升級程序之前，請確認已經上傳 UCenter 1.6.0 的全部文件和目錄</p>
...
<?php
	...
	if($total) {
		...
		@touch($lock_file);
		echo "<font style='font-size:20px;'>升級完成</font>。<br>原短消息表(".UC_DBTABLEPRE."pms表)已經不再使用，建議您在測試短消息功能及數據沒有問題之後，刪除此表。<br>將advanced/uc_client中的文件覆蓋您的uc_client客戶端。";
	}
